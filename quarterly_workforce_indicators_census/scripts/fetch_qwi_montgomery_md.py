...
# Montgomery County, Maryland (state: 24, county: 031)
# QWI API endpoint for employment data
url = "https://api.census.gov/data/timeseries/qwi/se?get=Emp&for=county:031&in=state:24&key=a19b29f73f2fb20843c8e5835a8d4c85c18467e4"

def fetch_qwi_data():
    """Fetch QWI data for Montgomery County, MD"""
    try:
        print("Fetching Quarterly Workforce Indicators data for Montgomery County, MD...")
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
        
        # Save raw data
        with open('montgomery_md_qwi_response.json', 'w') as file:
...
def convert_to_proper_json():
    ...
    try:
        # Read the raw data
        with open('montgomery_md_qwi_response.json', 'r') as file:
            data = json.load(file)
        ...
        with open('montgomery_md_qwi_response_proper.json', 'w') as file:
...
def main():
    ...
    print("Montgomery County, Maryland")
    print("=" * 60)
