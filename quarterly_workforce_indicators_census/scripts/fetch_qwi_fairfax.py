...
# Fairfax County, Virginia (state: 51, county: 059)
# QWI API endpoint for employment data
url = "https://api.census.gov/data/timeseries/qwi/se?get=Emp&for=county:059&in=state:51&key=a19b29f73f2fb20843c8e5835a8d4c85c18467e4"

def fetch_qwi_data():
    """Fetch QWI data for Fairfax County"""
    try:
        print("Fetching Quarterly Workforce Indicators data for Fairfax County, VA...")
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
        
        # Save raw data
        with open('fairfax_qwi_response.json', 'w') as file:
...
def convert_to_proper_json():
    ...
    try:
        # Read the raw data
        with open('fairfax_qwi_response.json', 'r') as file:
            data = json.load(file)
        ...
        with open('fairfax_qwi_response_proper.json', 'w') as file:
